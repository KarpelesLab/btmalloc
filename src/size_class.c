...
/* size -> class lookup table. Index by ceil(size/16)-1 = (size-1)>>4. The
 * lookup itself (btm_size_to_sc) is inlined in internal.h; this file just
 * builds the table. */
uint8_t btm_sc_lut[BTM_SC_LUT_ENTRIES];

void btm_size_class_init(void) {
    for (size_t k = 0; k < BTM_SC_LUT_ENTRIES; k++) {
